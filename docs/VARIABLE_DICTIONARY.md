...
| `echo_lv_systolic_wall_stress_g_cm2` | Систолическое напряжение стенки ЛЖ | GM | lv_geometry | numeric_continuous | г/см² | geometry_outcome_or_mediator | physiological numeric range; define after audit | TBD after data audit | LV geometry / morphometry variable: Систолическое напряжение стенки ЛЖ (по формуле Гроссмана) |  |
...
| `merged_lv_deformation_cv_pct` | CV Деформации (СФУ/GLS) | Reg_Mech | mechanics_outcome | numeric_continuous | % | mechanics_outcome | strictly positive | Merged from DICOR CV SFU and Echo CV GLS | Объединенный коэффициент вариации деформации ЛЖ (взятый по модулю) | |
